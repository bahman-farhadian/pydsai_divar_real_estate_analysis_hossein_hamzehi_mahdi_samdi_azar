...
| `reports/models/` | Model metadata plus locally generated trained model binaries. |
...
The repository ships the executed HTML reports, figures, logs, runtime summary, and model metadata. Generated data tables and trained model binaries are run artifacts; `make run` recreates them from the Zstandard source dataset.
