...
| `reports/data/` | Generated CSV/Parquet datasets, predictions, metrics, and summaries. This directory is regenerated locally and is not tracked. |
| `reports/figures/` | High-resolution generated figures. |
| `reports/models/` | Model metadata. Heavy trained model binaries are regenerated locally and are not tracked. |
...
The committed deliverables include HTML reports, figures, logs, runtime summary, and model metadata. Generated data tables/datasets and heavy model binaries are ignored because they are reproducible from the committed Zstandard source dataset:
