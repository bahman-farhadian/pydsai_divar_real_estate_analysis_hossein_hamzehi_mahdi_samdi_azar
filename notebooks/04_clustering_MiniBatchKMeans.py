# %% [markdown]
# # Phase 4: Clustering Analysis (MiniBatchKMeans)
#
...
df_cluster[export_cols].to_csv(DATA_PROCESSED / 'clustering_assignments_minibatch.csv', index=False)
print(f"Saved: clustering_assignments_minibatch.csv ({len(df_cluster):,} rows)")
...
cluster_profiles_export.to_csv(DATA_PROCESSED / 'cluster_profiles_minibatch.csv')
print(f"Saved: cluster_profiles_minibatch.csv")
...
# ### Files Created
#
# - `clustering_assignments_minibatch.csv` - Full data with cluster labels
# - `cluster_profiles_minibatch.csv` - Statistics per cluster
# - `clustering_summary_minibatch.csv` - Summary table
